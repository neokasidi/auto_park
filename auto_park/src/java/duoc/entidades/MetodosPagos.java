...
@NamedQueries({
    ...
    @NamedQuery(name = "MetodosPagos.findById", query = "SELECT m FROM MetodosPagos m WHERE m.id = :id"),
    @NamedQuery(name = "MetodosPagos.findByName", query = "SELECT m FROM MetodosPagos m WHERE m.name = :name")})
public class MetodosPagos implements Serializable {
    ...
    @Column(name = "name")
    private String name;
    @OneToMany(mappedBy = "metodoId")
    private List<Tickets> ticketsList;

    public MetodosPagos() {
    }

    public MetodosPagos(Integer id) {
        this.id = id;
    }
    ...
    public String toString() {
        return "duoc.entidades.MetodosPagos[ id=" + id + " ]";
    }
